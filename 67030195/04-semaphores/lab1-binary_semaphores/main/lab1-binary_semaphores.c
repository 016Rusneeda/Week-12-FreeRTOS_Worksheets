...
// ✅ เพิ่มตัวนับอีเวนต์ของ Producer
...
void producer_task(void *pvParameters) {
    ...
    while (1) {
        ...
        event_counter++;
        ESP_LOGI(TAG, "🔥 Producer: Generating event #%" PRIu32, event_counter);
...
void timer_event_task(void *pvParameters) {
    ...
    while (1) {
        if (xSemaphoreTake(xTimerSemaphore, portMAX_DELAY) == pdTRUE) {
            stats.timer_events++;
            ESP_LOGI(TAG, "⏱️  Timer: Periodic timer event #%" PRIu32, stats.timer_events);
            ...
            if ((stats.timer_events % 5U) == 0U) {
                ESP_LOGI(TAG, "📊 Stats - Sent:%" PRIu32 ", Received:%" PRIu32 ", Timer:%" PRIu32 ", Button:%" PRIu32,
...
void button_event_task(void *pvParameters) {
    ...
    while (1) {
        if (xSemaphoreTake(xButtonSemaphore, portMAX_DELAY) == pdTRUE) {
            ...
            ESP_LOGI(TAG, "🔘 Button: Press detected #%" PRIu32, stats.button_presses);
...
void monitor_task(void *pvParameters) {
    ...
    while (1) {
        ...
        ESP_LOGI(TAG, "  Producer Events: %" PRIu32, stats.signals_sent);
        ESP_LOGI(TAG, "  Consumer Events: %" PRIu32, stats.signals_received);
        ESP_LOGI(TAG, "  Timer Events:    %" PRIu32, stats.timer_events);
        ESP_LOGI(TAG, "  Button Presses:  %" PRIu32, stats.button_presses);
...
void app_main(void) {
    ...
    if (xBinarySemaphore && xTimerSemaphore && xButtonSemaphore) {
        ...
        // Install GPIO ISR service (ใช้ 0 ก็ได้ ถ้าต้องการ IRAM สามารถใช้ ESP_INTR_FLAG_IRAM)
